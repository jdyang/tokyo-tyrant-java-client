 ...
 */
public class RDB extends AbstractDB {
	/**
	...
	/**
	 * Set the {@link Transcoder} to be used to transcode keys. Not required:
	 * @{link StringTranscoder} is the default.
	 */
	...
	/**
	 * Set the {@link Transcoder} to be used to transcode values. Not required:
	 * @{link StringTranscoder} is the default.
	 */
	...
	/**
	 * Store a record. If a record with the same key exists in the database, it
	 * is overwritten.
	 * 
	 * @param key
	 *            specifies the key.
	...
	/**
	 * Store a new record. If a record with the same key exists in the database,
	...
	/**
	 * Concatenate a value at the end of the existing record. If there is no
	 ...
	 * left. If there is no corresponding record, a new record is created.
	 * 
	 * @param key
	 *            specifies the key.
	 * @param value
	 *            specifies the value.
	 * @param width
	 *            specifies the width of the record.
	 * @return If successful, the return value is true, else, it is false.
	 */
	public boolean putshl(Object key, Object value, int width) {
		return execute(new Putshl(keyTranscoder, valueTranscoder, key, value,
				width));
	}

	public boolean putshl(Object key, Object value, int width,
			Transcoder valueTranscoder) {
		return execute(new Putshl(keyTranscoder, valueTranscoder, key, value,
				width));
	}

	/**
	 * Store a record without response from the server If a record with the same
	 ...
	 *         record. {@code null} is returned if no record corresponds.
	 */
	 ...
	 * 
	 * @param keys
	 *            specifies an array containing the retrieval keys.
	 * @return If successful, the return value is the map contains corresponding
	 *         values, else, it is {@code null}. As a result of this method,
	 *         keys existing in the database have the corresponding values and
	...
	/**
	 * Initialize the iterator. The iterator is used in order to access the key
	...
	/**
	 * Get the next key of the iterator. It is possible to access every record
	 ...
	 * access.
	 * 
	 * @return If successful, the return value is the next key, else, it is
	 *         {@code null}. {@code null} is returned when no record is to be
	...
	/**
	 * Get forward matching keys. Note that this method may be very slow because
	 * every key in the database is scanned.
	 * 
	 * @param prefix
	 *            specifies the prefix of the corresponding keys.
	 * @param max
	 *            specifies the maximum number of keys to be fetched. If it is
	 *            not defined or negative, no limit is specified.
	 * @return The return value is an array of the keys of the corresponding
	 *         records. This method does never fail and return an empty list
	...
	/**
	 * Add an integer to a record. If the corresponding record exists, the value
	 ...
	 *            specifies the key.
	 * @param num
	 *            specifies the additional value. If it is not defined, 0 is
	 *            specified.
	 * @return If successful, the return value is the summation value, else, it
	...
	/**
	 * Add a real number to a record. If the corresponding record exists, the
	 ...
	 * @param key
	 *            specifies the key. If it is not defined, an empty string is
	 *            specified.
	 * @param value
	 *            specifies the value. If it is not defined, an empty string is
	 *            specified.
	 * @param opts
	 *            specifies options by bitwise or: {@link RDB#XOLCKREC} for
	 *            record locking, {@link RDB#XOLCKGLB} for global locking. If it
	 *            is {@code 0}, no option is specified.
	 * @return If successful, the return value is the value of the response or
	 *         {@code null} on failure.
	 */
	public Object ext(String name, Object key, Object value, int opts) {
		return execute(new Ext(keyTranscoder, valueTranscoder, name, key,
				value, opts));
	}
	 ...
	 * 
	 * @param params
	 *            specifies the string of the tuning parameters. If it is not
	 *            defined, it is not used.
	 * @return If successful, the return value is true, else, it is false.
	 */
	...
	/**
	 * Copy the database file. The database file is assured to be kept
	 * synchronized and not modified while the copying or executing operation is
	 * in progress. So, this method is useful to create a backup file of the
	 * database file.
	 * 
	 * @param path
	 *            specifies the path of the destination file. If it begins with
	 *            {@code @}, the trailing substring is executed as a command
	 *            line.
	 * @return If successful, the return value is true, else, it is false. False
	 ...
	 * 
	 * @param path
	 *            specifies the path of the update log directory.
	 * @param ts
	 *            specifies the beginning time stamp in microseconds.
	 * @param opts
	 *            specifies options by bitwise-or: {@link RDB#ROCHKCON} for
	 ...
	 * 
	 * @param host
	 *            specifies the name or the address of the server. If it is
	 *            {@code null}, replication of the database is disabled.
	 * @param port
	 *            specifies the port number.
	 * @param ts
	 *            specifies the beginning timestamp in microseconds.
	 * @param opts
	 *            specifies options by bitwise-or: {@link RDB#ROCHKCON} for
	 *            consistency checking.
	 * @return If successful, the return value is true, else, it is false.
	 ...
	 * @param opts
	 *            specifies options by bitwise-or: {@link RDB#MONOULOG} for
	...
	/**
	 * Execute the command. Use the default value transcoder.
	 * 
	 * @param <T>
	 *            the type of the return value of the command.
	 * @param command
	 ...
	 * 
	 * @param command
	 *            the command to receive response.
	 */
	...
	static class SocketConnection implements Connection {
		...
		public void write(ChannelBuffer buffer) throws IOException {
			...
			buffer.readBytes(b);
			// In blocking-mode, a write operation will return only after
			// writing all of the requested bytes.
